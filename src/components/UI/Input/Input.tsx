...
export function Input<T>({
  ...
  const load = (file: File | null) => {
    if (file) {
      ...
      reader.onload = () => {
        // @ts-ignore
        !!reader.result && onChange(reader.result.toString());
      };
    }
  };
  ...
  return (
    <div
      className={
        "container-input " +
        (type === InputTypeEnum.FILE ? "file " : "") +
        (type === InputTypeEnum.PASSWORD ? "password " : "") +
    ...
    >
      <div className="wrapper-input">
        {type !== InputTypeEnum.FILE ? (
          // @ts-ignore
          <div className={"label " + (isFocus || value.length ? "focus" : "")}>
            {label}
            {required ? "*" : ""}
        ...
        <input
          ...
          value={
            type === InputTypeEnum.FILE
              ? undefined
              : type === InputTypeEnum.NUMBER && value == 0
                ? undefined
                : value
          }
          required={required}
        ...
        {type === InputTypeEnum.FILE && !value && (
          <div className="load_btn">Загрузить</div>
        )}
      </div>
